...
def screenshot_full():
    ...
    with sync_playwright() as p:
        ...
        screenshot_path = os.path.join(OUTPUT_DIR, "tiktok_full_upload.png")
        page.screenshot(path=screenshot_path, full_page=True)
        print(f"Full page screenshot saved to {screenshot_path}")
        
        # Print details of buttons
        buttons = page.query_selector_all("button")
        for idx, btn in enumerate(buttons):
            text = btn.text_content() or ""
            if "Post" in text or "Đăng" in text:
                print(f"Post Button #{idx}: html='{btn.evaluate('el => el.outerHTML')}'")
